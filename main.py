...
async def generate_report(request: PromptRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if detected_intent == "chat":
        ...
        try:
            ...
            result = {
                "sql": None,
                "message": f"I couldn't process your request due to an error while communicating with the ERP: {e}",
                "tokens_used": 0,
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            if config.get("erp_type") == "odoo":
                ...
                if resp.status_code == 200 and data.get("result", {}).get("uid"):
                    return {
                        "token": data["result"].get("session_id", username + "-auth-token"),
                        "email": username
                    }
        ...
        traceback.print_exc()
        raise HTTPException(status_code=502, detail=f"Failed to connect to ERP: {str(exc)}")
