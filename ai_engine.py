...
You specialize in writing complex, optimized, production-safe SQL queries
for Odoo ERP (PostgreSQL backend).
You MUST use PostgreSQL specific SQL syntax. Do not use MariaDB or MySQL functions.

Database Schema Rules for Odoo:
- Odoo models are stored as tables with underscores (no 'tab' prefix). E.g., `sale.order` becomes `sale_order`.
- Table names and column names should be used directly as simple underscores.
- Every table has standard Odoo columns:
  - `id` (SERIAL/INT) is the Primary Key for all tables.
  - `create_date` (TIMESTAMP), `write_date` (TIMESTAMP), `create_uid` (INT), `write_uid` (INT).
- Relationship types:
  - Many2one: Stored as a column (e.g., `partner_id` in `sale_order` links to `res_partner.id`).
  - One2many: No column on parent; child has a many2one back to parent (e.g. `sale_order_line.order_id` links to `sale_order.id`).
  - Many2many: Stored in a separate relation table, usually named `modelA_modelB_rel` with columns `modelA_id` and `modelB_id`.
- Most transactional data is in `sale_order`, `purchase_order`, `account_move` (Invoices & Journal Entries since v13+), `stock_move`, `stock_quant`.

Odoo Product Architecture:
- `product_template` stores the general product definition (name, description, type, list_price).
- `product_product` stores the specific variant and links to the template via `product_tmpl_id`.
- **IMPORTANT**: To get the product name, you must join `product_product` to `product_template` on `product_product.product_tmpl_id = product_template.id`. The `name` field is NOT on `product_product`.

Odoo Stock Intelligence:
- `stock_quant` stores the current on-hand quantity per product and per location.
- `stock_quant.product_id` links to `product_product.id`.
- `stock_quant.location_id` links to `stock_location.id`.
- To get current "Warehouse Stock" or "Physical Stock", always filter by `stock_location.usage = 'internal'`.
- `stock_move` stores the history of stock movements (transfers, sales, purchases).

Odoo Financial Intelligence:
- Since v13, `account_move` handles both Invoices and Journal Entries.
- `move_type` distinguishes them: 'out_invoice' (Sales Invoice), 'in_invoice' (Purchase), 'out_refund' (Sales Credit Note/Return), 'in_refund' (Purchase Return), 'entry' (Journal Entry).
- `state` is usually 'draft', 'posted', or 'cancel'.
- `account_move_line` stores the individual lines for every move, linked via `move_id`.

Odoo Translation Intelligence:
- In recent Odoo versions, many text fields (like `name` in `product_template`, `partner_name`, etc.) are stored as **JSONB** objects for multi-language support.
- When selecting these fields, you MUST extract the value for the language to avoid returning a raw JSON object. 
- ALWAYS use the PostgreSQL JSONB extraction operator `->>` with a language key (assume `'en_US'` by default unless the prompt suggests otherwise).
- Example: `product_template.name ->> 'en_US' AS product_name`.

PostgreSQL Performance & Syntax Rules:
- STRICTLY USE PostgreSQL functions! (e.g., COALESCE, TO_CHAR, CURRENT_DATE, AGE, CONCAT).
- Use `->>` to extract text from JSONB fields. 
- Use `ILIKE` for case-insensitive matching.
- Date filtering: Use `date_trunc('month', create_date)` or `create_date::date`.
- For any amount, total, or currency fields, return the RAW numeric values. Do NOT use formatting functions to add currency symbols in the SQL.

Security & Safety:
- Only generate SELECT queries.
- SQL must start directly with SELECT or WITH.
- A middleware validator appends `LIMIT 1000` to all queries by default.
- IF and ONLY IF the user explicitly asks for "all records", "everything", or "no limit", you MUST include the comment `/* NO_LIMIT */` immediately after `SELECT` or `WITH`.
...
def _get_dynamic_system_prompt_segments(client_id: str, app_name: str = None, erp_type: str = "erpnext") -> str:
    ...
    try:
        ...
        for s in segments:
            # Filter out ERPNext-specific rules if the client is using Odoo
            if erp_type == "odoo" and ("is_return" in s.prompt_text.lower() or "tab" in s.prompt_text):
                continue
...
def _extract_referenced_tables(sql_text, erp_type="erpnext"):
    ...
    for match in _SQL_TABLE_PATTERN.finditer(sql_text):
        ...
        if erp_type == "erpnext":
            ...
            # For Odoo/Other, we don't have a strict prefix but we should avoid SQL keywords 
            keywords = {"SELECT", "FROM", "JOIN", "WHERE", "GROUP", "ORDER", "LIMIT", "AS", "ON", "WITH", "LEFT", "RIGHT", "INNER", "OUTER"}
            if table_name.upper() not in keywords and table_name not in tables:
                tables.append(table_name)
    return tables
...
def _find_unknown_tables(sql_text, local_schema):
    ...
    if erp_type == "odoo":
        # For Odoo, we are flexible with dots vs underscores in the validator
        # to handle transitions between model names and table names.
        normalized_available = {t.replace(".", "_") for t in available_tables}
        return [table for table in _extract_referenced_tables(sql_text, erp_type) if table.replace(".", "_") not in normalized_available]
...
def _build_effective_user_prompt(user_prompt, history):
    ...
    for relation in clarification.get("relations", []):
        ...
        # ERPNext specific join logic integration
        if parent_table.startswith("tab"):
            ...
            lines.append(
                ...
                f"and `{child_table}`.`parenttype` = '{parent_doctype}'."
            )
        else:
             # Odoo or other join logic integration (assuming FK is on child)
             # e.g. sale_order_line.order_id = sale_order.id
             fk_field = f"{parent_table}_id"
             lines.append(
                f"- `{child_table}` must join to `{parent_table}` with "
                f"`{child_table}`.`{fk_field}` = `{parent_table}`.`id`."
             )
...
def build_non_report_response(user_prompt, intent):
    ...
    if intent == "chat":
        if "your name" in prompt_lower or "who are you" in prompt_lower:
            return "I am your ERP AI reporting assistant. I can help build reports, dashboards, and forecasting queries for your ERP data."
        if "how are you" in prompt_lower:
            return "I am ready to help. Ask me for a report, dashboard, KPI, or forecast."
        if "thank" in prompt_lower:
            return "You're welcome. Ask me for any report or dashboard when you're ready."
        return "Hello. I can help you build reports, dashboards, KPIs, and forecasting queries."

    if intent == "clarification_needed":
        if _ORDINAL_COLUMN_PATTERN.search(prompt_lower):
            return "I need one detail before I change that report. Which column do you want there instead?"
        return "I can help with reporting, but I need a bit more context. Tell me which report or dataset you want to change, or describe the result you want."

    return "I am focused on ERP reporting and analytics. Ask me for a report, KPI, dashboard, trend, comparison, or forecast from your ERP data."
...
def generate_sql(user_prompt, history=None, client_id="DEMO_CLIENT_123", app_name=None, currency="USD", currency_symbol="$"):
    ...
    if erp_type == "odoo":
        dynamic_system_prompt = ODOO_SYSTEM_PROMPT + f"\n\nCurrency context: {currency}"
    else:
        dynamic_system_prompt = ERPNEXT_SYSTEM_PROMPT.format(currency=currency)
...
def generate_chat_response(user_prompt, history=None):
    system_prompt = (
        "You are a helpful and professional ERP AI data assistant. "
...
def auto_extract_context_override(client_id: str, message_id: int, original_prompt: str, feedback_comment: str) -> dict | None:
    ...
    system_prompt = """You are an ERP data analyst assistant. A user submitted negative feedback on an AI-generated ERP report.
