document.addEventListener('DOMContentLoaded', () => {
    ...
    function showCustomConfirm(message, title = "Confirm Action", confirmLabel = "Delete", confirmClass = "btn-danger") {
        return new Promise((resolve) => {
            confirmModalTitle.textContent = title;
            confirmModalMessage.textContent = message;
            confirmModalSubmit.textContent = confirmLabel;

            // Reset classes to avoid conflicts from previous calls
            confirmModalSubmit.className = confirmClass; 
    ...
    if (refreshSchemaBtn) {
        refreshSchemaBtn.addEventListener('click', async () => {
            const confirmed = await showCustomConfirm(
                'Are you sure you want to refresh the ERP schema? This might take a few seconds.',
                'Refresh Schema',
                'Refresh',
                'btn-primary'
            );
            ...
            const originalHTML = refreshSchemaBtn.innerHTML;
            refreshSchemaBtn.disabled = true;
            refreshSchemaBtn.innerText = 'Refreshing...';

            try {
                const res = await fetch(`/api/schema/refresh?client_id=${encodeURIComponent(CLIENT_ID)}`, { method: 'POST' });
                const data = await res.json();
                if (data.status === 'success') {
                    showToast(`Schema refreshed! ${data.available_doctypes} tables identified.`, 'success');
                } else {
                    showToast(data.detail || data.message || 'Refresh failed', 'error');
                }
            } catch (err) {
                console.error(err);
                showToast('Network error during schema refresh.', 'error');
            } finally {
                refreshSchemaBtn.disabled = false;
                refreshSchemaBtn.innerHTML = originalHTML;
            }
        });
    }
});
